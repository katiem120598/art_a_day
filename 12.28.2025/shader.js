...
let bassThresh = 190.0;
let midsThresh = 110.0;
let trebleThresh = 80.0;
...
function draw() {
  if (micReady && fft) {
    ...
    if (bassTrans){
      ...
      rvals[storedind] = 1.0;
      gvals[storedind] = 0.0;
      bvals[storedind] = (Math.random()*155.0+100.0)/255.0;
    ...
    if (midsTrans){
      ...
      yvec[storedind] = Math.random()*height;
      bval = Math.random();
      rvals[storedind] = max(0.0,bval-50.0/255.0);
      gvals[storedind] = 1.0;
      bvals[storedind] = bval;
      timevec[storedind] = millis()/1000.0;
      storedind = (storedind + 1) % 5;
    }
    ...
    if (trebleTrans){
      xvec[storedind] = Math.random()*width;
      yvec[storedind] = Math.random()*height;
      rvals[storedind] = Math.random();
      gvals[storedind] = Math.random();
      bvals[storedind] = 1.0;
      timevec[storedind] = millis()/1000.0;
      storedind = (storedind + 1) % 5;
    }
