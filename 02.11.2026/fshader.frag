...
vec4 rays_wave(vec4 color, vec4 background, vec2 position, float radius, float rayCount, float ray_length, sampler2D sound, vec2 uv);
vec4 rays_spec(vec4 color, vec4 background, vec2 position, float radius, float rayCount, float ray_length, sampler2D sound, vec2 uv);
...
void main() {
    ...
    vec4 color_wave = mix(vec4(0.0,0.0,0.0,1.0),vec4(0.0,0.0,0.0,1.0),d);
    vec4 color_spec = mix(vec4(0.0,0.0,0.0,1.0),vec4(0.0,0.0,0.0,1.0),d);

    
    float RADIUS = 0.23;
    float RAY_LENGTH = .5;

    color_wave = rays_wave(vec4(1.0), color_wave, vec2(aspect/2.0, 1.0/2.0), RADIUS, RAYS, RAY_LENGTH, u_waveform, uv);
    color_spec = rays_spec(vec4(1.0), color_spec, vec2(aspect/2.0, 1.0/2.0), RADIUS, RAYS, RAY_LENGTH, u_waveform, uv);

    gl_FragColor = 1.0*color_wave*vec4(1.0,0.0/250.0,50.0/255.0,1.0)+1.0*color_spec*vec4(50.0/255.0,0.0/255.0,1.0,1.0);
}

vec4 rays_wave(vec4 color, vec4 background, vec2 position, float radius, float rayCount, float ray_length, sampler2D sound, vec2 uv)
{
    float inside = (2.5 - ray_length) * radius; //empty part of circle
    float outside = radius - inside; //rest of circle
    float base_length = 8.5;
    float circle = 10.0*M_PI*inside; //circle lenght
    for(int i = 1; i <= int(RAYS); i++)
    {
        if (float(i)>rayCount) break;

        float len = outside*(20.0*(texture2D(sound, vec2(float(i)/rayCount, 0.0)).x)-(base_length));
...
vec4 rays_spec(vec4 color, vec4 background, vec2 position, float radius, float rayCount, float ray_length, sampler2D sound, vec2 uv)
{
    float inside = (2.5 - ray_length) * radius; //empty part of circle
    float outside = radius - inside; //rest of circle
    float base_length = 10.5;
    float circle = 4.0*M_PI*inside; //circle lenght
    for(int i = 1; i <= int(RAYS); i++)
    {
        if (float(i)>rayCount) break;
